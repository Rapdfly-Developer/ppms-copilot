...
Use only documented information. Cite visit sources throughout.`,
  ...
  DIFFERENTIAL_DIAGNOSIS: `Task: List possible diagnostic considerations for the treating ophthalmologist's own review, based strictly on documented symptoms, history, vitals, and existing diagnoses in this patient's record. This is NOT a diagnosis — it is a structured aid to the doctor's own clinical reasoning, and the doctor makes the final diagnostic decision.
...
Do not imply access to results you do not have. Every consideration must be based only on documented symptoms, HPI, vitals-as-recorded, and existing documented diagnoses.

If the documented record does not contain enough information to support any consideration, do not guess — say so explicitly using the exact sentence given below and stop.

## Possible Considerations for Review
List each consideration as its own block, ordered from most to least supported by the documented record:
...
Rules for this list:
- Use ONLY "Low consideration" or "Moderate consideration" as the confidence label. NEVER use "high probability," "likely," "confirmed," "definite," or any other language implying certainty.
- Every item MUST cite a specific documented finding. Do not include any consideration without a citation — an uncited consideration is not permitted.
- Do not pad the list. List only as many considerations as the documented record actually supports — this may be one, or none.
- If no documented finding supports any diagnostic consideration, write only this exact sentence and nothing else in this section: "The documented record does not contain sufficient findings to support any diagnostic considerations at this time."
...
**IMPORTANT NOTICE — FOR DOCTOR REVIEW ONLY:** This list was generated by AI from the documented patient record only. It is NOT a diagnosis and must NOT be used as one. It does not include laboratory, imaging, or investigation result data — only documented symptoms and history. These are possible considerations for the treating doctor's own clinical judgement. The doctor is solely responsible for the final diagnostic decision and must not act on this output without independent clinical assessment.

State only what the record documents. Do not add, infer, or assume any clinical detail not present in the documentation.`,
