...
// Structured view of one suggested-investigation block — the same blocks
// validateInvestigationGuidance() already parsed and approved, not a second
// independent parse. Unlike DifferentialDiagnosisItem, `rationale` is kept —
// it's the substance of this capability's output, not just a compact badge.
...
// Structured view of INVESTIGATION_GUIDANCE's result. Part of the eager
// consolidated call, same as PlanGuidanceResult.
//
// `investigationsSummary` is not generated by this capability's own
// prompt/validator — attached in service/copilot-generate.ts from the SAME
// already-validated INVESTIGATIONS_SUMMARY section generated in this same
// consolidated call. No new AI call, no new validation for this field. Absent
// (not an error) when INVESTIGATIONS_SUMMARY itself failed or was empty.
//
// `suggestedInvestigations` IS new AI content — correlative-only by prompt
// and validator design (see IMPERATIVE_LANGUAGE_PATTERNS and the new
// DIRECTIVE_LANGUAGE_PATTERNS in validation/response.ts). [] means "nothing
// to suggest", not an error.
export type InvestigationGuidanceResult = {
  investigationsSummary?: string;
  suggestedInvestigations: SuggestedInvestigationItem[];
};

// One Differential Diagnosis item's reason text, cited verbatim — NOT part
// of DifferentialDiagnosisItem/PLUGIN_DIFFERENTIAL_UPDATE (that contract is
// already agreed with PPMS Core and stays untouched: name/confidence/source
// only). This is a separate, internal-only citation used exclusively to
// assemble DiagnosisComparisonResult.differentialDiagnosisReasoning — built
// entirely in application code from the same-generation Differential
// Diagnosis output, never a second AI call, never re-worded.
export type DifferentialReasonCitation = {
  name: string;
  reason: string;
};

// Structured view of DIAGNOSIS_COMPARISON's result. Part of the eager
// consolidated call.
//
// `plausibility` is genuinely new AI content — a bounded judgment on whether
// the current visit's documented diagnosis is consistent with documented
// findings, never a second opinion or override. Absent (not an error) when
// no diagnosis is documented for the current visit.
//
// `differentialDiagnosisReasoning` is NOT generated by this capability's own
// prompt/validator — assembled in service/copilot-generate.ts entirely from
// the SAME already-validated Differential Diagnosis section's own reason
// text, threaded via DifferentialReasonCitation. No new AI call, no
// re-composition. Absent (not an error) when Differential Diagnosis itself
// failed, or resolved to its own empty/insufficient-evidence case.
...
export type PlanGuidanceResult = {
  ...
  // Not generated by PLAN_GUIDANCE's own prompt/validator — attached in
  // service/copilot-generate.ts from the SAME already-validated
  // FOLLOW_UP_SUMMARY section generated in this same consolidated call. No
  // new AI call, no new validation: the exact text the Follow-up tab already
  // shows, referenced a second time. Absent (not an error) when
  // FOLLOW_UP_SUMMARY itself failed or was empty for this visit.
